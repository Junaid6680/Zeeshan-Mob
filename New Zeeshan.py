...
# --- PDF Generation Function (For Sending to Customers) ---
    ...
    if st.button("➕ Add Item to Bill"):
        if item_name:
            st.session_state.temp_items.append({
                "Item": item_name, "Qty": item_qty, "Price": item_price, "Total": item_qty * item_price
            })
...
# --- Full Ledger & Backup Section ---
st.divider()
st.header("📒 Full Ledger Overview & Search")
...
st.download_button(
    label="💾 Download All Customers Backup (Excel/CSV)",
    data=csv_data,
    file_name=f"Zeeshan_Ledger_Backup_{pd.Timestamp.now().strftime('%d-%m-%Y')}.csv",
    mime="text/csv"
)
