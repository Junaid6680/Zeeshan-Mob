...
# Naya feature: Sales History for Reports
...
with col_sidebar:
    ...
    if st.button("Confirm Payment (No Bill)"):
        if pay_only > 0:
            bill_id = f"PAY-{int(time.time())}"
            old_b = cust_data['balance']
            st.session_state.customer_db[selected_cust]['balance'] -= pay_only
            
            # Save to history for reports
            new_entry = pd.DataFrame([[bill_id, pd.Timestamp.now(), selected_cust, 0, pay_only]], 
                                    columns=["Bill No", "Date", "Customer", "Total", "Paid"])
            st.session_state.sales_history = pd.concat([st.session_state.sales_history, new_entry], ignore_index=True)
            
            pdf_p = create_pdf(bill_id, selected_cust, cust_data['phone'], [], 0, old_b, pay_only, is_only_payment=True)
            st.download_button("📥 Download Payment Receipt", pdf_p, f"Receipt_{selected_cust}.pdf", "application/pdf")
            st.success(f"Payment Received!")
            st.rerun()
...
with col_main:
    ...
    if st.session_state.temp_items:
        ...
        if st.button("✅ Finalize & Save Bill"):
            bill_id = f"ZMA-{int(time.time())}" # Unique Bill ID
            old_b = cust_data['balance']
            st.session_state.customer_db[selected_cust]['balance'] = (curr_total + old_b) - paid_today
            
            # Naya feature: Save to sales history for reports
            new_sale = pd.DataFrame([[bill_id, pd.Timestamp.now(), selected_cust, curr_total, paid_today]], 
                                    columns=["Bill No", "Date", "Customer", "Total", "Paid"])
            st.session_state.sales_history = pd.concat([st.session_state.sales_history, new_sale], ignore_index=True)
            
            pdf_d = create_pdf(bill_id, selected_cust, cust_data['phone'], st.session_state.temp_items, curr_total, old_b, paid_today)
            st.download_button(f"📥 Download Bill {bill_id}", pdf_d, f"Bill_{selected_cust}.pdf", "application/pdf")
            st.session_state.temp_items = []
            st.rerun()
    else:
...
if not st.session_state.sales_history.empty:
    ...
    daily_sale = history[history['Date'].dt.date == now.date()]['Total'].sum()
    weekly_sale = history[history['Date'] > (now - pd.Timedelta(days=7))]['Total'].sum()
    monthly_sale = history[history['Date'] > (now - pd.Timedelta(days=30))]['Total'].sum()
    yearly_sale = history[history['Date'].dt.year == now.year]['Total'].sum()
    
    r1, r2, r3, r4 = st.columns(4)
    r1.metric("Today's Total Sale", f"Rs. {daily_sale}")
    r2.metric("Weekly Sale", f"Rs. {weekly_sale}")
    r3.metric("Monthly Sale", f"Rs. {monthly_sale}")
    r4.metric("Yearly Sale", f"Rs. {yearly_sale}")
    
    with st.expander("View Full Sales History"):
        st.dataframe(history.sort_values(by="Date", ascending=False), use_container_width=True)
else:
